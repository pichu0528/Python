...
are given two timestamps of one such post that a user can see on his newsfeed in the following format:
...
In the first query, when we compare the time in UTC for both the time stamps, we see a difference
of 7 hours. which is 7 x 3600 seconds or 25200 seconds.

Similarly, in the second query, time difference is 5 hours and 30 minutes for time zone adjusting
for that we have a difference of 1 day and 30 minutes. Or 
...
def time_delta(t1, t2):
    ...
    difference_in_seconds = str(round(abs(time1-time2).total_seconds()))
